namespace Gomoku
{
	...
	internal class BotUser : IBot
	{
		private readonly Random _rnd;
		private readonly Board _board;
		private readonly Gomoku _game;

		public BotUser(Gomoku game, Board board)
		{
			_game = game;
			_board = board;
			_rnd = new Random();
		...
		private int GetWeight(Point pos)
		{
			...
			if (_board.IsConnectAmount(pos, CellValue.Bot, 2)) weight += 10;
			if (_board.IsConnectAmount(pos, CellValue.User, 2)) weight += 20;
			if (_board.IsConnectAmount(pos, CellValue.Bot, 3)) weight += 40;
			if (_board.IsConnectAmount(pos, CellValue.User, 3)) weight += 80;
			if (_board.IsConnectAmount(pos, CellValue.Bot, 4)) weight += 160;
			if (_board.IsConnectAmount(pos, CellValue.User, 4)) weight += 320;
			if (_board.IsConnectAmount(pos, CellValue.Bot, 5)) weight += 640;
			if (_board.IsConnectAmount(pos, CellValue.User, 5)) weight += 1280;
		...
		private int GetOneNeighborWeight(Point pos, Point dir)
		{
			...
			if (_board.GetValue(newp) == CellValue.Bot) return 1;
			if (_board.GetValue(newp) == CellValue.User) return 2;
			return 0;
		}
	}
}
